<template>
    <div class="edu_exp">
        <h3 class="hr">教育经历</h3>
        <div class="school">
            <input class="list_box" v-if="!schoolName" type="text" placeholder="毕业院校..." @blur="schoolName=$event.target.value">
            <span v-else @click="schoolName=''">{{schoolName}}</span>
            <input class="list_box" v-if="!courseInfo" type="text" placeholder="专业名称--本科/专科..." @blur="courseInfo=$event.target.value">
            <span v-else @click="courseInfo=''">{{courseInfo}}</span>
            <input class="list_box" v-if="!studyTime" type="text" placeholder="起始年月-毕业年月..." @blur="studyTime=$event.target.value">
